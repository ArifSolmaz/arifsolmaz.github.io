...
def download_image(url: str, paper_id: str) -> str | None:
    ...
    try:
        ...
        print(f"    ‘£Ù Downloaded: {temp_path} ({size/1024:.1f} KB)")
        return temp_path
...
def get_figure_for_paper(paper: dict) -> str | None:
    ...
    print(f"≠˛Ù¿ Getting image for {paper_id}...")
...
def generate_paper_card(paper: dict) -> str | None:
    ...
    try:
        ...
        print(f"    ‘£Ù Generated paper card: {temp_path}")
        return temp_path
...
def format_tweet_thread_premium(paper: dict, page_url: str, hashtags: list[str], limit: int) -> tuple[str, str]:
    ...
    parts.append(f"≠˛Ù‰ {title}")
    parts.append(f"≠˛ÊÒ {author_str}")
    ...
    tweet2 = f"≠˛Ù‰ arXiv: {link}\n≠˛Ù˚ Full summary: {summary_link}\n\n{hashtag_str}"
...
def main():
    ...
    paper_type = "≠˛¨… EXOPLANET" if paper_to_tweet.get("is_exoplanet_focused", True) else "≠˛ˆ° GENERAL"
    ...
    if api_v1:
        ...
        if figure_path:
            ...
            if media_id:
                media_ids = [media_id]
                print(f"≠˛Ù© Image will be attached to tweet!")
            else:
                print("‘‹·¥©≈ Image upload failed, tweeting without image")
        else:
            print("≠˛Ù‰ No image available, tweeting text only")
    ...
    if tweet1_id:
        ...
        if tweet2_id:
            ...
            print("\n‘‹·¥©≈ Tweet 1 posted but Tweet 2 failed")
            tweeted_ids.add(paper_to_tweet["id"])
            tweeted_data["tweeted_ids"] = list(tweeted_ids)
            save_tweeted(tweeted_data)
    else:
        print("\n‘ÿÓ Failed to post tweet")
        sys.exit(1)
